...
import {Gateway, Service, ServiceList} from '@api/backendapi';
...
@Component({
  selector: 'kd-gateway-detail',
  templateUrl: './template.html',
})
export class GatewayComponent implements OnInit, OnDestroy {
  private gatewayDetailSubscription_: Subscription;
  private gatewaySetName_: string;
  ...
  constructor(
      private readonly gateway_: NamespacedResourceService<Gateway>,
  ...
  ngOnInit(): void {
    this.gatewaySetName_ = this.state_.params.resourceName;
    this.gatewayDetailSubscription_ =
        this.gateway_
            .get(EndpointManager.resource(Resource.gateway, true).detail(), this.gatewaySetName_)
            .startWith({})
            .subscribe((d: Gateway) => {
              this.gateway = d;
              // this.data_ = this.map(d);
  ...
  ngOnDestroy(): void {
    this.gatewayDetailSubscription_.unsubscribe();
  }
}
